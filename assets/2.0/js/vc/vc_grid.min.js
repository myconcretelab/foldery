...
/**
 * ZK Choles 
 * This theme just added rtl support for VC Grid
 * added settings option to data-vc-grid-settings
 * js_composer/include/templates/shortcodes/vc_basic_grid.php
 * added:  $this->grid_settings['rtl'] = is_rtl();
 * making change in js
...
! function($) {
    ...
    }, vcGridStyleAll.prototype.showItems = function(delay,callback) {
        var $this = this, $els = this.$content.find(".vc_grid-item" + this.filterValue);
        this.setIsLoading();
        var animation = this.$content.closest(".vc_grid-container").data("initial-loading-animation");
        if(Number.isInteger(delay))
        {
            setTimeout(function () {
                vcGridSettings.addItemsAnimation = animation,
                    ...
                    $this.unsetIsLoading(), $(window).trigger("grid:items:added", $this.$el)
...
! function($) {
    ...
    }, vcGridStyleLoadMore.prototype.showItems = function(delay,callback) {
        var $this=this, $els = this.$content.find(".vc_grid_filter-item:not(.vc_visible-item):lt(" + this.settings.items_per_page + ")");
        this.setIsLoading();
        var animation = this.$content.closest(".vc_grid-container").data("initial-loading-animation");
        if(Number.isInteger(delay))
        {
...
! function($) {
    $.waypoints("extendFn", "vc_grid-infinite", function(options) {
        ...
        this.$el = this.grid.$el, this.$content = this.$el, this.setIsLoading(), this.grid.ajax({}, this.addItems)
    }, vcGridStyleLazy.prototype.showItems = function(delay,callback) {
        var _this=this,$els = this.$content.find(".vc_grid_filter-item:not(.vc_visible-item):lt(" + this.settings.items_per_page + ")");
        this.setIsLoading();
        var animation = this.$content.closest(".vc_grid-container").data("initial-loading-animation");
        if(Number.isInteger(delay))
        {
        ...
        if(self.$el.length > 0)
        {
            var att_class = self.$el.attr('class'),class_prefix = 'zk-loading-delay-';
            att_class.split(' ').forEach(function (item,index) {
                if(item.indexOf(class_prefix) == 0)
                {
                    loading_delay = parseInt(item.substring(class_prefix.length));
                    return false;
                }
            });
        }
        if(!Number.isInteger( loading_delay ) || loading_delay < 0)
            loading_delay = 3000;
        this.$content.unbind("waypoint").waypoint("vc_grid-infinite", {
            ...
            handle: {
                load: function(opts) {
                    self.showItems(loading_delay,function () {
                        self.checkNext(opts);
...
! function($) {
    vcGridStyleAllMasonry = function(grid) {
        ...
        showItems: function(delay,callback) {
            var $els = this.$content.find(".vc_grid-item" + this.filterValue),
                self = this,is_over_delay = false,is_image_loaded = false;
            this.setIsLoading(), $els.imagesLoaded(function() {
                if(!is_over_delay && Number.isInteger(delay))
                {
            ...
            if(Number.isInteger(delay))
            {
                setTimeout(function () {
                    ...
                    vcGridSettings.addItemsAnimation = animation,
                        ...
                        self.setItems($els),
                    self.filtered && (self.filtered = !1, self.setMasonry()), self.unsetIsLoading(),
                        window.vc_prettyPhoto(), $(window).trigger("grid:items:added", self.$el);
                    if(typeof callback == 'function')
                        callback();
                },delay);
            }
            ...
            if(Number.isInteger(delay))
            {
                setTimeout(function () {
                    ...
                    self.filtered && (self.filtered = !1, self.setMasonry(), self.initScroll(), window.vc_prettyPhoto()),
                        self.unsetIsLoading(), $(window).trigger("grid:items:added", self.$el);
                    if(typeof callback == 'function')
                        callback();
                },delay);
            }

...
// Si on a un folder à afficher, on commence son analyse    
    ...
    if (foldery_is_media_folder($folder)) :
        ...
        $meta = foldery_media_get_folder_meta($folder->getId());
    ?>
        ...
        if ( function_exists('foldery_lightbox_activate') && $folder->getCnt()) :
            echo foldery_lightbox_activate(do_shortcode('[masonry folder_id="' . $id_gallery . '"]'));
            $first = 1;
    endif;            
    ...
    if(is_array($children)) :
        ...
        foreach ($children as $child) :
            if($child->getCnt()) :
                ...
                echo do_shortcode('[masonry folder_id="' . $child->getId() . '"]') . "\n";
                $first = 1;
            endif;
        endforeach;
        $out = ob_get_clean();
        // Activate links in output.
        if (function_exists('foldery_lightbox_activate')) $out = foldery_lightbox_activate($out); 
        // Display output.
        echo $out;
    endif;
    endif;

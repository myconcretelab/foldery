...
class CsCoreControl
{
    ...
    private function imegesselect($params){
        ...
        <div class="image-field csfield clearfix">
            <ul>
                ...
                <li data-value="<?php echo esc_attr($key); ?>" class="<?php if($params['value'] == $key) { echo 'active'; } ?>"><img alt="image" src="<?php echo esc_url($image) ?>"></li>
                <?php endforeach; ?>
            </ul>
    ...
    private function icon($params){
        ...
        <div class="icon-field">
            <input type="text" style="width: 170px;" class="thickbox" alt="#TB_inline?amp;inlineId=field_icon"
            name="<?php echo esc_attr($params['id']); ?>"
            id="<?php echo esc_attr($params['id']); ?>"
            value="<?php if(isset($params['value'])){ echo esc_attr($params['value']);} ?>"
            placeholder="<?php if(isset($params['placeholder'])){ echo esc_attr($params['placeholder']);} ?>" />
        ...
        <div class="images-field clearfix">
            ...
            foreach ($selected as $value):
                ...
                <li class="items" data-id="<?php echo esc_attr($value); ?>" style="background-image:url(<?php echo esc_url($attachment_image[0]);?>);background-size:cover;">
                    <i class="edit dashicons dashicons-plus-alt" title="<?php _e('Replace Image', 'foldery'); ?>"></i>
                    <i class="remove dashicons dashicons-dismiss" title="<?php _e('Remove Image', 'foldery'); ?>"></i>
                </li>
            <?php endif; endforeach; ?>
            <?php if($params['field'] != 'single'): ?>
                <li class="items" data-id=""><i class="add dashicons dashicons-plus-alt" title="<?php _e('Add Image', 'foldery'); ?>"></i></li>

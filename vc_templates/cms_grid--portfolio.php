...
<div class="cms-grid-wraper cms-grid-portfolio <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>">
    <?php if($atts['element_title'] !='' || $atts['element_sub_title'] !='' || $atts['element_title_desc'] !='') : ?>
        ...
        <?php switch ($element_title_layout) {
            ...
            ?>
                ...
                <div class="cms-element-desc"><?php foldery_allowed_html($atts['element_sub_title']); ?></div>
            <?php
                break;
                case '2' :
            ?>
    ...
    <div class="<?php echo esc_attr($atts['grid_class']);?>">
        <?php
        $posts = $atts['posts'];
        $size = ($atts['layout']=='basic') ? 'large' : 'blog-masonry';
        $d = 0;
        while($posts->have_posts()){
            ...
            ?>
            <div class="<?php echo esc_attr($atts['item_class']).' '.$cls.' nopaddingall overlay-wrap';?>" data-groups='[<?php echo implode(',', $groups);?>]'>
                <?php 
